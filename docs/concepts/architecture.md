...
returns rows already in primary-key (or indexed-value) order, so `ORDER BY <pk>`
...
- A primary-key point lookup is a direct key fetch; a PK **range / prefix /
  `ORDER BY pk`** is a contiguous byte range with no secondary structure and no sort.
...
row (delete-here + insert-there) instead of updating it in place. That is why
...
min/max statistics and strong file and row-group **pruning** in the warehouse. The

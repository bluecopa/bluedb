...
## Pinned semantics
- **id_already_failed:** the transient codes (`debit_account_not_found`, `credit_account_not_found`, `pending_transfer_not_found`, `exceeds_credits`, `exceeds_debits`, `debit_account_already_closed`, `credit_account_already_closed`) — and all terminal failures and `linked_event_failed` — BURN the id. TB does this so a retry can never produce a different outcome. `created` and the `exists`/`exists_with_different_*` family NEVER burn. Burned id on a later attempt → `id_already_failed` (code 24, checked after the exists family at 23, before flags 25). Applies to transfers only (accounts have no `id_already_failed`).

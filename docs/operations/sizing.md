...
and N clients sustain ~N × 40/s — **until the node hits its knee** (below). Latency
per write stays ≈ the flush interval regardless of N; only throughput scales.
...
(These hold only while clients ≤ the node's knee — they are the *linear region*,
...
**past 256 clients (~9,600 inserts/s)** with p50 still ~27 ms — the knee was beyond
...
A hard, predictable ceiling — a write-concurrency limit (a semaphore / max
in-flight writes, or a connection cap) set at the measured knee, so excess clients
queue or get `503` instead of thrashing — is a natural next lever but is **not yet
a configurable option**. Until then, enforce limits at your load balancer / client
pool.
...
- **Storage is the object store — elastic.** Not a node-sizing concern (nodes are

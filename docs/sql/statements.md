...
- `SET default_null_order = 'nulls_first' | 'nulls_last'`: controls where
  ...
  Accepted on `/sql` as a per-session, per-tenant knob (and on `/admin/sql`).
- `PRAGMA lakehouse_mirror[...]` and `PRAGMA lakehouse_target_file_bytes = <n>`
  ...
  **applied**, not no-ops. Accepted on `/sql` and `/admin/sql`.
- Other engine-config `SET` / `PRAGMA` knobs are accepted as **no-ops** on
  `/admin/sql` (so scripts written for other engines run unchanged); on `/sql`
  only the documented knobs above are intercepted and any other `SET` is
  rejected (use `/admin/sql` for arbitrary session setup).

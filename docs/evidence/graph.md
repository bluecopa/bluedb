...
- **Nodes are `TEXT`, weights are 64-bit `INTEGER`.** Nodes are implicit — the
  union of all `src`/`dst` — there is no separate node table.
...
Both traversals run read-only over the adjacency indexes (no write lease — they
work on a read replica). `directed: true` (the default) follows `out` edges only;
...
- **No similarity / vector search.** This is an exact graph store (adjacency,
  weights, maximin) — no HNSW or vector index.

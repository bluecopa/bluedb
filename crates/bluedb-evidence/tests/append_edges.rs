...
use bluedb_evidence::{EdgeDelta, EdgeOp, EdgeRef, EntryInput, Evidence, Graph, Merge};
use bluedb_sql::{prefix_upper_bound, Database, Keyspace};
use slatedb::{object_store::memory::InMemory, Db};

async fn db() -> Database {
    let d = Db::open("ae-test", Arc::new(InMemory::new()))
        .await
        .unwrap();
...
async fn count_graph_keys(database: &Database) -> (usize, usize, usize) {
    ...
    (
        count_tag(database, 0x1C).await,
        count_tag(database, 0x1D).await,
        count_tag(database, 0x1E).await,
    )
}
...
async fn append_materializes_edges_on_plain_chain() {
    ...
    let entry = EntryInput {
        ...
        payload: vec![],
        at: String::new(),
        edges: vec![upsert_edge("lin", "A", "B", 1)],
    };
    ev.append("c", vec![entry], None).await.unwrap();
    assert_eq!(count_graph_keys(&database).await, (1, 1, 1));
}
...
async fn multi_entry_batch_same_identity_composes() {
    ...
    let e2 = EntryInput {
        ...
        payload: vec![],
        at: String::new(),
        edges: vec![upsert_edge("lin", "A", "B", 9)],
    };
...
async fn idempotent_replay_does_not_double_apply_edges() {
    ...
    let entry = EntryInput {
        ...
        payload: vec![],
        at: String::new(),
        edges: vec![upsert_edge("lin", "A", "B", 3)],
    };
    ev.append("c", vec![entry.clone()], Some("k1"))
        .await
        .unwrap();
...
async fn standalone_delete_removes_append_created_edge() {
    ...
    let entry = EntryInput {
        ...
        payload: vec![],
        at: String::new(),
        edges: vec![upsert_edge("lin", "A", "B", 5)],
    };
    ev.append("c", vec![entry], None).await.unwrap();

...
async fn scan_ids(database: &Database, sql: &str) -> Vec<i64> {
    let mut g = Glue::new(database.connection_serialized());
    let out = rest_sql::execute_sql(&mut g, sql, &[], false).await.unwrap();
    ids_from(out)
}
...
async fn like_infix_is_accelerated_and_matches_scan() {
    ...
    assert_eq!(scan_ids(&database, sql).await, vec![1, 3], "baseline sanity");
}
...
async fn like_without_trigram_index_passes_through() {
    let db = Arc::new(Db::open("trgm-like-noidx", Arc::new(InMemory::new())).await.unwrap());
    let database = Database::new(db);
    let fts = FtsEngine::new();
    {
        let mut g = Glue::new(database.connection_serialized());
...
async fn like_prefix_and_suffix_infix_match_scan() {
    ...
    assert_eq!(engine_ids(&fts, &database, suffix).await, scan_ids(&database, suffix).await);

    // Prefix anchor.
    let prefix = "SELECT id FROM docs WHERE body LIKE 'overdue%'";
    assert_eq!(engine_ids(&fts, &database, prefix).await, scan_ids(&database, prefix).await);
    assert_eq!(scan_ids(&database, prefix).await, vec![3], "only row 3 starts with 'overdue'");
}
...
async fn like_negated_passes_through() {
    ...
    assert_eq!(engine_ids(&fts, &database, sql).await, scan_ids(&database, sql).await);
    assert_eq!(scan_ids(&database, sql).await, vec![2], "only the non-overdue row");
}
...
async fn like_correct_across_seal_durable() {
    let db = Arc::new(Db::open("trgm-like-seal", Arc::new(InMemory::new())).await.unwrap());
    let database = Database::new(db);
    ...
    let sql = "SELECT id FROM docs WHERE body LIKE '%overdue%'";
    assert_eq!(engine_ids(&fts, &database, sql).await, vec![1, 3], "pre-seal live");
    ...
    assert_eq!(engine_ids(&fts, &database, sql).await, vec![1, 3], "post-seal still 1 and 3");

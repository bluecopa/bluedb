...
async fn gc_keys_deletes_superseded_splits_after_compaction() {
    ...
    let s1 = writer
        .append(
            &mut manifest,
            schema.clone(),
            vec![doc(id_f, "a", body_f, "alpha")],
        )
        .expect("s1");
    blob.put(&s1.blob_key, s1.split_bytes.clone().into())
        .await
        .expect("put s1");
    let s2 = writer
        .append(
            &mut manifest,
            schema.clone(),
            vec![doc(id_f, "b", body_f, "beta")],
        )
        .expect("s2");
    blob.put(&s2.blob_key, s2.split_bytes.clone().into())
        .await
        .expect("put s2");
    ...
    blob.put(&result.blob_key, result.split_bytes.clone().into())
        .await
    ...
    let n = gc_keys(blob.as_ref(), &result.superseded_keys)
        .await
        .expect("gc");
    ...
    let orphans = gc_orphaned_splits(blob.as_ref(), &new_manifest)
        .await
        .expect("orphan scan");
...
async fn gc_orphaned_splits_deletes_unreferenced_blobs() {
    ...
    for (id, term) in [("a", "alpha"), ("b", "beta"), ("c", "gamma")] {
        let r = writer
            .append(
                &mut manifest,
                schema.clone(),
                vec![doc(id_f, id, body_f, term)],
            )
            .expect("append");
        blob.put(&r.blob_key, r.split_bytes.clone().into())
            .await
            .expect("put");
    ...
    let orphans = gc_orphaned_splits(blob.as_ref(), &advanced)
        .await
        .expect("gc orphans");
    assert_eq!(
        orphans,
    ...
    assert!(gc_orphaned_splits(blob.as_ref(), &advanced)
        .await
        .unwrap()
        .is_empty());
}

...
// Adapted for bluedb: copied verbatim from
// `quickwit-storage/src/cache/byte_range_cache.rs`; only the `crate::metrics`
// imports (here and in the test module) were repointed to `super::metrics` for
// the module layout. The cache logic, including the `CacheMetrics` /
// `SingleCacheMetrics` counter calls, is unchanged — they hit the local
// dependency-free metrics stub.
...
use super::metrics::{CacheMetrics, SingleCacheMetrics};

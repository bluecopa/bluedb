...
// Adapted for bluedb: only `crate::` references (the `read_only_directory!`
// macro and the test module's `crate::ReadOperation`) were repointed to
...
mod tests {
    ...
    fn test_debug_proxy_atomic_read() -> tantivy::Result<()> {
        ...
        let operations: Vec<super::ReadOperation> = debug_proxy.drain_read_operations().collect();

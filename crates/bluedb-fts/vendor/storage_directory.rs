...
// Adapted for bluedb: `quickwit_storage::{OwnedBytes, Storage}` repointed to the
// local `super::` modules, and `crate::read_only_directory!` to `super::`. The
// upstream `uri()` returned `&quickwit_common::uri::Uri`; here the trimmed
// `Storage` trait returns `&str`, so `Uri` is no longer imported and
// `StorageDirectory::uri()` returns `&str`.

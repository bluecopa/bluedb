...
async fn new_guarded_glue() -> Glue<SlateDbStorage> {
    let db = Arc::new(Db::open("composite-pk-guarded", Arc::new(InMemory::new())).await.unwrap());
...
async fn seed(glue: &mut Glue<SlateDbStorage>) {
    exec(glue, "CREATE TABLE t (a INTEGER, b TEXT, payload TEXT, PRIMARY KEY (a, b))")
        .await
        .unwrap();
    for (a, b, p) in [(1, "m", "p1"), (1, "x", "p2"), (1, "z", "p3"), (2, "a", "p4")] {
        exec(glue, &format!("INSERT INTO t (a, b, payload) VALUES ({a}, '{b}', '{p}')"))
            .await
            .unwrap();
    }
}
...
async fn point_prefix_and_range_lookups() {
    ...
    let got = rows(exec(&mut glue, "SELECT payload FROM t WHERE a = 1 AND b = 'x'").await.unwrap());
    assert_eq!(got, vec![vec![Value::Str("p2".into())]]);

    // Leading-prefix lookup (all rows with a = 1), in key order.
    let got = rows(exec(&mut glue, "SELECT b FROM t WHERE a = 1 ORDER BY a, b").await.unwrap());
    assert_eq!(
        got,
        vec![
...
async fn select_star_hides_the_surrogate() {
    let mut glue = new_glue().await;
    seed(&mut glue).await;
    let got = rows(exec(&mut glue, "SELECT * FROM t WHERE a = 1 AND b = 'x'").await.unwrap());
    // Three user columns (a, b, payload) — NOT the hidden __bluedb_pk.
    assert_eq!(got, vec![vec![Value::I64(1), Value::Str("x".into()), Value::Str("p2".into())]]);
}
...
async fn update_and_delete_by_composite_key() {
    ...
    exec(&mut glue, "UPDATE t SET payload = 'updated' WHERE a = 1 AND b = 'x'").await.unwrap();
    let got = rows(exec(&mut glue, "SELECT payload FROM t WHERE a = 1 AND b = 'x'").await.unwrap());
    assert_eq!(got, vec![vec![Value::Str("updated".into())]]);

    exec(&mut glue, "DELETE FROM t WHERE a = 1 AND b = 'm'").await.unwrap();
    let got = rows(exec(&mut glue, "SELECT b FROM t WHERE a = 1 ORDER BY a, b").await.unwrap());
    assert_eq!(got, vec![vec![Value::Str("x".into())], vec![Value::Str("z".into())]]);
}
...
async fn composite_lookups_are_index_served_on_a_guarded_connection() {
    ...
    assert!(exec(&mut glue, "SELECT payload FROM t WHERE a = 1 AND b = 'x'").await.is_ok());
    assert!(exec(&mut glue, "SELECT b FROM t WHERE a = 1 ORDER BY a, b").await.is_ok());
    assert!(exec(&mut glue, "SELECT b FROM t WHERE a = 1 AND b >= 'x' ORDER BY a, b").await.is_ok());

    // Control: a genuinely non-indexed filter is still rejected (guardrail live).
    assert!(
        exec(&mut glue, "SELECT a FROM t WHERE payload = 'p1'").await.is_err(),
